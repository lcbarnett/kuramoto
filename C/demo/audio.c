...
int audio(int argc, char *argv[])
{
	...
	CLAP_ARG(T,      double, 1.0,       "total integration time (secs)");
	CLAP_ARG(f,      double, 44100.0,   "sampling frequency (Hz)");
	CLAP_ARG(b,      int,    24,        "sampling bits");
	CLAP_ARG(dt,     double, 1.0/f,     "integration step size");
	CLAP_ARG(wmean,  double, MIDDLE_C,  "oscillator frequencies mean (Hz)");
	CLAP_ARG(wrdev,  double, 0.5,       "oscillator frequencies rel. dev.");
	CLAP_ARG(Kmean,  double, 0.1*f/N,   "coupling constants mean (Hz)");
	CLAP_ARG(Krdev,  double, 1.0,       "coupling constants rel. dev.");
	CLAP_ARG(Isdev,  double, 0.1,       "input noise intensity (zero for deterministic)");
...
//	const double rad2Hz = f/(2.0*M_PI);
	const double Hz2rad = (2.0*M_PI)/f;
	...
	double* const a = calloc(N*N,sizeof(double)); // coupling constants
	...
	for (size_t i=0; i<N; ++i) {
		w[i] = dt*Hz2rad*wmean*(1.0+wrdev*randn()); // scale frequencies by dt
	}

	// random phase shifts (uniform distribution)

	for (size_t i=0; i<N; ++i) {
		for (size_t j=0; j<N; ++j) {
			if (i == j) {
				a[N*i+j] = 0.0; // no "self-connections"!
			}
			else {
				a[N*i+j] = 0.01*2.0*M_PI*randu();
			}
		}
	}
	...
	for (size_t i=0; i<N; ++i) {
		for (size_t j=0; j<N; ++j) {
			if (i == j) {
				...
				K[N*i+j] = dt*Hz2rad*Kmean*(1.0+Krdev*randn()); // scale coupling constants by dt
			}
		}
	}
	...
	if (Isdev > 0.0) {
		for (size_t k=0; k<N*n; ++k) {
			h[k] = sqrtdt*Hz2rad*Isdev*randn(); // scale input by sqrt(dt) [cf. Ornstein-Uhlenbeck process]
		}
	}
	else {
	...
	if (RK4) {
		...
		kuramoto_rk4pl(N,n,w,K,a,h,kbuff);
		free(kbuff);
	}
	else {
		kuramoto_eulerpl(N,n,w,K,a,h);
	}
	...
	for (size_t k=0; k<n; ++k) {
		...
		for (size_t i=0; i<N; ++i) {
			fprintf(fp," %17.8f",sin(f*h[N*k+i])); // oscillator signal (waveform)
		}
		fprintf(fp,"\n");
	}
	if (fclose(fp) != 0) {
